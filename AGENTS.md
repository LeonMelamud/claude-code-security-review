...
### 1. Tessl Registry (tessl.io)
To publish directly to the Tessl registry under the `need-skills` workspace:
1. **Bump the version** in `tile.json` (e.g., from `0.1.1` to `0.1.2`).
2. **Publish the tile**, skipping the built-in evals (since this repo uses a custom Python eval framework):
   ```bash
   tessl tile publish --skip-evals
   ```

### 2. skills.sh (Tessl ecosystem)
